...
class TobaccoLegalCompliance(Document):
    def get_fda(self):
        ...
        #         SELECT tlc.permit_number as '(Enter the T T B permit number.)',
        # tlc.phone as  '(Enter the telephone number including area code.)',
        # tlc.employer_identification_number as  "(Enter the employer's identification number.)",
        # tlc.email as '(Enter e mail address.)' ,
        # tlc.month as '(Enter the period of activity month.)',
        # tlc.year as   '(Enter the period of activity year.)',
        # CONCAT(tlc.address_line1,'\n',tlc.address_line2,'\n',tlc.zipcode,'\n',tlc.us_state ) as '(Enter the company name/address including Zip Code\).)',
        # tlc.legal_head as "Enter the contact person's name.)" ,
        # round(SUM(coalesce(PR.total_net_weight,0))*2.20462,2) as '(Enter box 31 monthly volume total for imported pipe tobacco.)',
        # case AC.account_type  when "Tax" then SUM(coalesce(PTC.base_tax_amount,0)) else 0 end as '(Enter box 38 monthly tax total for imported pipe tobacco.)',
        # 'OWNER' AS "(Enter authorizing official's title.)",
        # DATE_FORMAT(CURDATE(), '%m/%d/%Y') AS '(Enter date prepared, in a 2 digit month / 2 digit day / 4 digit year format.)'
        # FROM `tabTobacco Legal Compliance` tlc
        # INNER JOIN `tabPurchase Receipt` PR
        # ON PR.company = tlc.company
        # and PR.docstatus = 1
        ...
        # AND PRI.item_group in (select distinct name from `tabItem Group` where parent_item_group = 'TOBACCO')
        # LEFT JOIN `tabPurchase Taxes and Charges` AS PTC
    ...
    def get_55206(self):
        ...
        # CONCAT(tlc.address_line1,'\n',tlc.address_line2,'\n',tlc.zipcode,'\n',tlc.us_state ) as '2 PRINCIPAL BUSINESS ADDRESS Number Street City State and ZIP Code',
