	...
	if self.items:
		if self.items[0]:
			if self.items[0].delivery_note:
				...
				shipping_method_za,order_tracking_number,no_of_boxes_za = frappe.db.get_value('Delivery Note', delivery_note, ['shipping_method_za', 'order_tracking_number','no_of_boxes_za'])
